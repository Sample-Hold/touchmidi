...
// =============================================================================
function init() {
  var midi_port = 0; // MIDI output port number
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if (
      nodes[i].className == "row" ||
      nodes[i].className == "column" ||
      nodes[i].className == "main_column" ||
  ...
  for (var i = 0; i < but_nodes.length; i++) {
    ...
    if (node.className.indexOf("direct") > 0) type = "DIRECT";
    var colour = "#00FF00";
    var label = "";
  ...
  // Our GLOBAL singelton MIDI object
...
function eventMouseUp(e) {
  ...
  if (e.constructor.name != "MouseEvent") {
    for (var i = 0; i < e.changedTouches.length; i++) {
      ...
      if (active_widgets["id_" + changed_id])
        active_widgets["id_" + changed_id].callMouseUp();
...
function parseMinMax(node, widget) {
  if (node.hasAttribute("min")) {
    ...
    if (widget.constructor.name == "XYPad") {
      widget.xmin = min;
      widget.ymin = min;
    }
    widget.min = min;
    ...
    for (var i = 0; i < action_list.length; i++) {
      ...
      if (widget.constructor.name == "Button") {
        if (params.length > 2) {
          ...
          // User forgot to add parameters, only fallback is zeros
          action.nrpn_val_on = 0;
          action.nrpn_val_off = 0;
        }
      }
      action.nrpn_msb = parseInt(params[1].trim());
      action.nrpn_lsb = parseInt(params[2].trim());
      if (widget.max > 127) action.high_res = true;
